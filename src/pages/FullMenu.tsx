   ...
   return (
      <div className="min-h-screen bg-tertiary">
         <div className="sticky top-0 w-full z-50 bg-tertiary border-b border-neutral/10 py-5 px-6 lg:px-12 flex items-center justify-between shadow-sm">
            ...
            <span className="font-serif text-2xl font-semibold text-neutral absolute left-1/2 -translate-x-1/2">Menu</span>
            <div className="flex justify-end min-w-[80px]">
               <button onClick={onCheckout} className="relative p-2.5 bg-white rounded-full shadow-sm text-neutral hover:bg-soft transition-colors border border-neutral/5">
                  <ShoppingBag size={20} />
                  {totalItems > 0 && (
                     <span className="absolute -top-1.5 -right-1.5 bg-primary text-white text-[11px] font-bold w-5 h-5 flex items-center justify-center rounded-full shadow-sm border-2 border-tertiary">
                        {totalItems}
                     </span>
                  )}
               </button>
